    ...
    <div className="p-4 md:p-8 max-w-[1000px] mx-auto space-y-6 slide-up">
      ...
      <div className="bg-linear-to-r from-amber-600/20 via-amber-500/30 to-amber-600/20 border border-amber-500/40 rounded-2xl p-5 text-center">
        <div className="flex items-center justify-center gap-3">
          ...
          <h2 className="text-amber-500 font-bold text-2xl tracking-widest">WARNING</h2>
          <WarnTriangle size={36} className="alarm-flash" />
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          <div className="glass p-5 space-y-4">
            <h3 className="text-sm font-semibold text-muted uppercase tracking-wider">Tín Hiệu Cảnh Báo</h3>
            {[
              ...
              <div key={ind.key} className="flex items-center gap-4">
                ...
                <div>
                  <span className="text-sm font-medium text-strong">{ind.label}</span>
                  <span className={`block text-xs ${plc[ind.key] ? "text-amber-500 font-semibold" : "text-muted"}`}>
          ...
          <button {...momentary("Dung")}
            className="w-full py-3 bg-red-600/20 hover:bg-red-600/30 border border-red-500/40 text-red-500 font-bold text-sm rounded-xl
          ...
          <div className="glass p-5 space-y-3">
            <h3 className="text-sm font-semibold text-muted uppercase tracking-wider">Chi Tiết Cảnh Báo</h3>
            {alarms.map(a => (
              <div key={a.key} className={`flex items-start gap-3 p-3 rounded-lg transition-all ${plc[a.key] ? "bg-amber-500/10 border border-amber-500/20" : "bg-badge border border-border-subtle"}`}>
                <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${plc[a.key] ? "bg-amber-500 alarm-flash" : "bg-border-strong"}`} />
                <div>
                  <span className={`text-sm font-medium ${plc[a.key] ? "text-amber-500 font-bold" : "text-strong"}`}>{a.label}</span>
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="glass p-6">
            <h3 className="text-sm font-semibold text-muted mb-4 uppercase tracking-wider">Vị Trí Sự Cố</h3>
            <svg viewBox="0 0 480 300" className="w-full max-w-[520px] mx-auto">
              ...
              <path d="M120,60 Q150,48 180,60 Q210,72 240,60 Q270,48 300,60 Q330,72 350,60" stroke="var(--svg-coil-off)" strokeWidth="3" fill="none" />
              <circle cx="150" cy="90" r="14" fill="none" stroke="var(--svg-coil-off)" strokeWidth="2" />
              <circle cx="310" cy="90" r="14" fill="none" stroke="var(--svg-coil-off)" strokeWidth="2" />
              ...
              <rect x="215" y="211" width="30" height="8" fill="var(--svg-coil-off)" rx="3" />
              <rect x="120" y="248" width="40" height="22" rx="4" fill="var(--bg-btn-hover)" stroke="var(--svg-stroke)" />
              <text x="140" y="263" textAnchor="middle" fontSize="8" fill="var(--text-strong)" fontWeight="bold">M1</text>
              <rect x="300" y="248" width="40" height="22" rx="4" fill="var(--bg-btn-hover)" stroke="var(--svg-stroke)" />
          ...
          <div className="flex justify-end">
            <button onClick={onNavigateMain}
              className="px-6 py-3 bg-btn hover:bg-btn-hover border border-border-strong text-strong font-medium text-sm rounded-xl
